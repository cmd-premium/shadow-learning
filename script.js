...
// ——— In-app browser: content in a div (no iframe), address bar shows real URL, fetch via proxy in background ———
(function inAppBrowser() {
  var BROWSER_PROXY = "https://shadow-learning-production.up.railway.app";
  ...
  function proxyUrl(realUrl) {
    ...
    return base + "/browse?u=" + encodeURIComponent(realUrl);
  }
  ...
  function isProxyLink(href) {
    ...
    try {
      ...
      var origin = BROWSER_PROXY ? new URL(BROWSER_PROXY).origin : location.origin;
      return u.origin === origin && u.pathname.indexOf("/browse") !== -1 && (u.searchParams.get("u") || u.searchParams.get("url"));
    } catch (e) { return false; }
  }

  function getUrlFromProxyLink(href) {
    try {
      var u = new URL(href, location.href);
      return u.searchParams.get("u") || u.searchParams.get("url") || href;
    } catch (e) { return href; }
  }

  function loadInBrowser(realUrl) {
    if (!realUrl || !/^https?:\/\//i.test(realUrl)) return;
    if (browserUrl) browserUrl.value = realUrl;
    if (browserLoading) browserLoading.hidden = false;
    browserContent.innerHTML = "";
    fetch(proxyUrl(realUrl), { headers: { Accept: "text/html,*/*;q=0.8" } })
      ...
      .catch(function (err) {
        if (browserLoading) browserLoading.hidden = true;
        browserContent.innerHTML = "<div class=\"browser-error\"><p>Could not load this page.</p><p>" + (err.message || "Check your connection.") + "</p></div>";
      });
  }
  ...
  browserContent.addEventListener("click", function (e) {
    ...
    if (isProxyLink(a.href)) {
      e.preventDefault();
      loadInBrowser(getUrlFromProxyLink(a.href));
    }
  });
  ...
  browserContent.addEventListener("submit", function (e) {
    ...
    if (!action) return;
    e.preventDefault();
    var url = action.indexOf("/browse") !== -1 ? getUrlFromProxyLink(action) : action;
    try { url = new URL(url, "https://www.google.com").href; } catch (err) { return; }
    var method = (form.method || "get").toLowerCase();
    if (method === "get") {
      var params = new URLSearchParams(new FormData(form));
      url = url.replace(/#.*$/, "") + (url.indexOf("?") !== -1 ? "&" : "?") + params.toString();
      loadInBrowser(url);
    } else {
      fetch(proxyUrl(url), { method: "POST", body: new FormData(form) })
        ...
        .catch(function () {
          if (browserLoading) browserLoading.hidden = true;
          browserContent.innerHTML = "<div class=\"browser-error\"><p>Could not load this page.</p></div>";
        });
    }
  });
  ...
  function showPanel(page) {
    ...
    if (page === "browser") {
      ...
      if (!browserContent.innerHTML.trim()) loadInBrowser("https://www.google.com");
    }
